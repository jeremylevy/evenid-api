...
describe('models.middlewares.pre.save.'
         + 'sendNotificationToClients (Entity)', function () {
    ...
    it('notifies client which '
      + 'have asked for updated entity', function (done) {
        ...
        async.auto({
            ...
            updatePhoneNumber: ['createUserAuthorization',
                ...
                sendNotificationToClients('phone_numbers').call(phoneNumber, function (err) {
                    if (err) {
                        return cb(err);
                    }
                    ...
                    oauthNotifications.forEach(function (oauthNotification) {
                        ...
                        var isValidPhoneNumber = function (fakeID) {
                            return function (phoneNumber) {
                                ...
                                assert.strictEqual(phoneNumber.number,
                                                   updatedPhoneNumber.number);

                                assert.strictEqual(phoneNumber.phone_type,
                                                   updatedPhoneNumber.phone_type);

                                assert.strictEqual(phoneNumber.status, 'updated');
    ...
    it('doesn\'t notify client which '
       + 'don\'t handle update notification', function (done) {
        ...
        async.auto({
            ...
            // And one which must not be notified
            createClient: function (cb) {
                createClient.call({
            ...
            assertItHasNotUpdatedOauthNotifications: ['updatePhoneNumber', function (cb, results) {
                var client = results.createClient;
                var entityIDForUser = results.createEntityIDForUser;
            ...
            assertItHasNotCreatedOauthNotif: ['updatePhoneNumber', function (cb, results) {
                findLastOauthNotification(function (err, oauthNotification) {
                    var notifications = [];
    ...
    it('doesn\'t notify clients when only '
       ...
       + 'equals `old_phone_type`', function (done) {
        ...
        async.auto({
            ...
            assertItHasNotSetOauthNotifInSQS: ['updatePhoneNumber', function (cb, results) {
                findLastOauthNotification(function (err, oauthNotification) {
                    var notifications = [];
