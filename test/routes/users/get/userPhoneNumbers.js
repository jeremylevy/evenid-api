...
describe('GET /users/:user_id/phone-numbers', function () {
    ...
    it('responds with HTTP code 403 and `access_denied` error when '
       + 'non-app token try to get user phone numbers', function (done) {
    ...
    it('responds with HTTP code 403 and `access_denied` error when '
       + 'user try to get phone numbers which does not belong to him', function (done) {
    ...
    it('responds with HTTP code 200 and phone numbers when valid user', function (done) {
        // Positive lookahead assertion - Negative lookahead assertion
        var reg = new RegExp('(?=.*\\[\\{)(?=.*id)'
                             + '(?=.*number)(?=.*country)'
                             + '(?=.*phone_type)'
                             // Resp sent as
                             // {phone_numbers: ..., territories: ...}
                             + '(?=.*phoneNumbers)(?=.*territories)'
                             + '(?!.*"_id")(?!.*"__v")');
        
        makeARequest(200, reg, function (err, resp) {
            var body = !err && resp.body || {};
            var phoneNumbers = body.phoneNumbers;
            ...
            assert.ok(Type.is(phoneNumbers, Array)
