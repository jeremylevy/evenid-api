...
var getOauthClientRegisteredUsers = require('./getOauthClientRegisteredUsers');
...
module.exports = function (clientID, period, today, timezone, cb) {
    ...
    async.auto({
        ...
        aggregateRegisteredUsers: function (cb) {
            db.models.OauthClientRegisteredUser.aggregate(
                [
                    {
                        $match: {
            ...
            ).exec(function (err, registeredUsers) {
                if (err) {
                    return cb(err);
                }
        ...
        var nbOfRegisteredUsers = results.findNbOfRegisteredUsers;
        var registeredUsers = results.aggregateRegisteredUsers;
        ...
        registeredUsers.forEach(function (registeredUser) {
            stats[moment.tz(registeredUser.last_date, timezone)
                        .format(momentFormat)] = registeredUser;
        });
        ...
        for (var i = 0; i <= periodNb; ++i) {
            ...
            if (stats[createdFromFormated]) {
                ...
                stats[createdFromFormated] = stats[createdFromFormated].count;
            ...
            // check as boolean
            if (Type.is(previousCount, Number)) {
                stats[createdFromFormated] = previousCount;

                continue;
            }

            stats[createdFromFormated] = nbOfRegisteredUsers;
        }
